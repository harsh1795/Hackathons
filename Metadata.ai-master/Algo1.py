...
def L31pred():
    ...
    URL = 'DUMB.csv'
    DUMB = pandas.read_csv(URL)
    df_DUMB = DUMB[DUMB.columns[34:37] ] 
    df_test = data[list(data.columns[35:38])]
    frames = [ df_DUMB,df_test]
    df_WH = pandas.concat(frames)
    #ENCODING
    X_test = df_WH.to_dict('records')
...
def L32pred():
    ...
    URL = 'DUMB.csv'
    DUMB = pandas.read_csv(URL)
    df_DUMB = DUMB[DUMB.columns[39:42] ] 
    df_test = data[list(data.columns[40:43])]
    frames = [ df_DUMB,df_test]
    df_WH = pandas.concat(frames)
    #ENCODING
    X_test = df_WH.to_dict('records')
...
def L33pred():
    ...
    df_DUMB = DUMB[DUMB.columns[44:47]] 
    
    df_test = data[ list(data.columns[45:48])]
    frames = [ df_DUMB,df_test]
    df_WH = pandas.concat(frames)
